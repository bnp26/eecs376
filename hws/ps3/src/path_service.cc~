...
bool path_callback(custom_msgs::pathRequest& request, custom_msgs::pathResponse& response)
{
    ROS_INFO("callback activated");
    PathExecutor executor;

    unsigned int waypoints_size = request.path.poses.size();
    ROS_INFO("received path request with %u poses", waypoints_size);


    std::vector<std::pair<double, double> > cmds = executor.generate_commands(request.path.poses);
