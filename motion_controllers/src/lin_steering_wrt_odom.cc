...
#include "motion_controllers/steering_algorithm.h"

nav_msgs::Odometry current_odom_global1;
...
void SteeringController::getInitialState()
{
	const double UPDATE_RATE = 0.02;
	ros::Rate loop_timer(1 / UPDATE_RATE);
	ros::Subscriber sub = this->nh_.subscribe("/current_state", 1, &SteeringController::getInitialStateCallback, this);
	
	while(!this->is_init_state_received_ && ros::ok())
	{
		ros::spinOnce();
		loop_timer.sleep();
	}
...
void SteeringController::getInitialStateCallback(const nav_msgs::Odometry& init_state)
{
	ROS_INFO("current odom pose: x=%f, y=%f, z=%f", init_state.pose.pose.position.x,
                                                    init_state.pose.pose.position.y,
                                                    init_state.pose.pose.position.z);
	this->des_state_ = init_state;
	this->des_state_pose_ = init_state.pose.pose;
    this->des_state_vel_ = init_state.twist.twist.linear.x;
    this->des_state_omega_ = init_state.twist.twist.angular.z;
    this->des_state_x_ = init_state.pose.pose.position.x;
    this->des_state_y_ = init_state.pose.pose.position.y;
    this->des_state_quat_ = init_state.pose.pose.orientation;
    //odom publishes orientation as a quaternion.  Convert this to a simple heading
    this->des_state_phi_ = convertPlanarQuat2Phi(des_state_quat_); // cheap conversion from quaternion to heading for planar motion
	this->is_init_state_received_ = true;
}
...
void SteeringController::odomCallback(const nav_msgs::Odometry& odom_rcvd) {
    ...
    current_odom_global1 = odom_rcvd;
...
void SteeringController::lin_steering_algorithm() {
    ...
    odom_x_ = current_odom_global1.pose.pose.position.x;
    odom_y_ = current_odom_global1.pose.pose.position.y;
    odom_quat_ = current_odom_global1.pose.pose.orientation;
    ...
    // DEBUG OUTPUT...
    ROS_INFO("odom_x=%f, odom_y= %f des_x=%f, des_y=%f " ,odom_x_,odom_y_,des_state_x_,des_state_y_);
...
{ 
    ...
    ros::Rate sleep_timer(1 / UPDATE_RATE); //a timer for desired rate, e.g. 50Hz
