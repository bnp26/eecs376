#include "eecs376_ps6/pub_des_state.h"
...
DesStatePublisher::DesStatePublisher(ros::NodeHandle& nh) : nh_(nh) {
    ...
    current_pose_ = trajBuilder_.xyphi_to_pose_stamped(0,0,0);
...
void DesStatePublisher::initializeServices() {
    ...
    this->alarm_subscriber_ = nh_.subscribe("/lidar_alarm", 1, &DesStatePublisher::lidar_alarm_CB, this);
...
bool DesStatePublisher::appendPathQueueCB(eecs376_ps6::PathRequest& request, eecs376_ps6::PathResponse& response) {
...
void DesStatePublisher::lidar_alarm_CB(const std_msgs::Bool& alarm_msg)
{
    this->e_stop_trigger_ = alarm_msg.data;
    if(this->e_stop_trigger_)
    {
...
void DesStatePublisher::set_init_pose(double x, double y, double psi) {
    current_pose_ = trajBuilder_.xyphi_to_pose_stamped(x, y, psi);
}
...
void DesStatePublisher::pub_next_state() {
    ...
    if (e_stop_trigger_) {
        ...
        trajBuilder_.build_braking_traj(current_pose_, des_state_vec_, traj_pt_i_);
    ...
    switch (motion_mode_) {
        ...
        case HALTING: //e-stop service callback sets this mode
            ...
            des_psi_ = trajBuilder_.convert_planar_quaternion_to_phi(current_pose_.pose.orientation);
