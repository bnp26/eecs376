...
void TrajBuilder::build_braking_traj(odom_t current_state, std::vector<odom_t>& vec_of_states)
{
    odom_t desired_state = current_state;

    double initial_angular_velocity = current_state.twist.twist.angular.z;
    double inital_linear_velocity = current_state.twist.twist.linear.x;
    ...
    // stop all motion
    double& current_angular_vel = desired_state.twist.twist.angular.z;
    double& current_linear_vel = desired_state.twist.twist.linear.x;
    ...
    double max_linear_vel = inital_linear_velocity;
    if(inital_linear_velocity < 0.0)
    {
        min_linear_vel = inital_linear_velocity;
        max_linear_vel = 0.0;
    }

    double min_angular_vel = 0.0;
    double max_angular_vel = initial_angular_velocity;
    if(initial_angular_velocity < 0.0)
    {
        min_angular_vel = initial_angular_velocity;
        max_angular_vel = 0.0;
    }
    ...
    {
        desired_state.pose.pose = computed_curved_pose(desired_state, this->_dt);
    ...
    desired_state.twist.twist = this->_zero_twist;
    vec_of_states.push_back(desired_state);
