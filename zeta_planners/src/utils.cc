...
pose_t computed_curved_pose(const odom_t& prior_state, const double dt)
{
    ...
    //ROS_INFO("computing new pose from old pose and current twist");
    ...
    //ROS_INFO("currently moving at linear vel: %f, angular vel: %f", current_linear_vel, current_angular_vel);

    if(current_angular_vel != 0.0 && current_linear_vel != 0.0)
    {
    ...
    else if(current_angular_vel != 0.0)
    {
		pose.position.x = prior_x_position;
		pose.position.y = prior_y_position;
		pose.orientation = convert_planar_phi_to_quaternion(prior_angle + current_angular_vel * dt);
	}
	else
	{
		pose.position.x = prior_x_position + current_linear_vel * dt;
		pose.position.y = prior_y_position;
		pose.orientation = prior_state.pose.pose.orientation;
	}
    return pose;
}
